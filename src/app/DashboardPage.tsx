...
const DashboardPage = () => {
  ...
  const pendingCount = userSubmissions.filter(
    (item) =>
      item.status === 'AI Auto-Check' ||
      item.status === 'AI Approved' ||
      item.status === 'Heritage Check'
  ).length;
  ...
  const handleAddSubmission = async (newSubmission: NewSubmissionInput) => {
    ...
    try {
      ...
      const firestoreSubmission = {
        ...
        aiCategory: 'pending' as SubmissionCategory,
        moderationReason: '',
        heritageReviewNote: '',
        createdAtMillis,
      ...
      try {
        ...
        const moderationMessage = getReadableBackendError(moderationError);

        await updateDoc(doc(db, 'submissions', docRef.id), {
          status: 'AI Auto-Check',
          verificationStage: 'AI Auto-Check',
          aiCheckResult: 'System Error',
          aiCategory: 'system_error',
          moderationReason: moderationMessage,
          heritageReviewNote: moderationMessage,
        });
        ...
        setSuccessMessage('');
        setBackendError(
          `Submission was saved, but AI moderation could not complete: ${moderationMessage}`
        );
      }
    } catch (error) {
      setBackendError(getReadableBackendError(error));
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                ...
                <p className="text-stone-600 mt-2">
                  Track your cultural submissions through the verification
                  workflow.
                </p>
              </div>
              ...
              {!latestSubmission && (
                <div className="mt-6 bg-stone-50 border border-dashed border-stone-300 rounded-xl p-4 text-center">
                  <p className="text-sm text-stone-500">
                    No submissions yet. Click <strong>Upload New Item</strong>{' '}
                    to get started.
                  </p>
                </div>
              )}
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-stone-100 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-stone-500">
              Upload documents, photos, audio, or videos for preservation
              review.
            </p>
          </div>
